...
def send_email(mesage, recipient, * ,sender = 'university.help@gmail.com' ):
    if ('@' not in recipient or not (recipient.endswith('.com') or recipient.endswith('.ru') or recipient.endswith('.net'))) or \
       ('@' not in sender or not (sender.endswith('.com') or sender.endswith('ru') or sender.endswith('net'))):
        ...
        print(f'письмо успешн отправлено с адреса {sender} на адрес {recipient}')
    else :
